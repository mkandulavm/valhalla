...
thor::PathAlgorithm* thor_worker_t::get_path_algorithm(const std::string& routetype,
  ...
  // Alternates are generated by bidirectional A*. For time-dependent
  // depart_at/current requests, prefer bidirectional only when alternates are
  // explicitly requested so default behavior stays unchanged.
  const bool has_requested_alternates = options.has_alternates_case() && options.alternates() > 0;
  const bool is_timedep_depart_or_current =
      options.date_time_type() == Options::current || options.date_time_type() == Options::depart_at;
  const bool prefer_bidir_for_timedep_alternates =
      has_requested_alternates && is_timedep_depart_or_current;
  ...
  if (!origin.date_time().empty() && options.date_time_type() != Options::invariant &&
      !options.prioritize_bidirectional() && !prefer_bidir_for_timedep_alternates) {
